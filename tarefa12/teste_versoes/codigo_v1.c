// ===================================================================
// Versão 1 (V1) - Otimização com Vetorização SIMD
// ===================================================================
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void simulation_step_v1_corrigida() {
    double (*u)[N][N] = u_data;
    double (*v)[N][N] = v_data;
    double (*w)[N][N] = w_data;
    ...
    // CORREÇÃO: Paraleliza os loops i e j com collapse(2).
    // Cada thread fica responsável por um conjunto de "colunas" (laços em k).
    #pragma omp parallel for collapse(2) schedule(static)
    for (int i = 1; i < N - 1; ++i) {
        for (int j = 1; j < N - 1; ++j) {
            // A diretiva SIMD agora se aplica ao loop k, que está "livre"
            // dentro do trabalho de cada thread.
            #pragma omp simd
            for (int k = 1; k < N - 1; ++k) {
