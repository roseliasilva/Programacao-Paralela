// ===================================================================
// Versão Otimizada Final (Cache Blocking)
// ===================================================================
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// Define o tamanho da aresta do bloco. Este valor pode ser ajustado (ex: 8, 16, 32).
// 16 é um bom ponto de partida.
...
void simulation_step() {
    ...
    // A paralelização agora acontece nos loops de bloco.
    // Cada thread fica responsável por um conjunto de blocos.
    ...
    for (int i_block = 1; i_block < N - 1; i_block += BLOCK_SIZE) {
        for (int j_block = 1; j_block < N - 1; j_block += BLOCK_SIZE) {
            for (int k_block = 1; k_block < N - 1; k_block += BLOCK_SIZE) {
                ...
                // A condição composta com '&&' funciona perfeitamente aqui, pois não há 'simd'.
...
int main(int argc, char *argv[]) {
    ...
    printf("Iniciando Versão Otimizada (Cache Blocking)...\n");
