// ===================================================================
// Versão 3 (V3) - Combinação de Tiling e SIMD
// ===================================================================
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void simulation_step_v3_corrigida() {
    double (*u)[N][N] = u_data;
    double (*v)[N][N] = v_data;
    double (*w)[N][N] = w_data;
    ...
    // A paralelização continua nos loops de BLOCo. Cada thread
    // pega um ou mais blocos para processar.
    ...
    for (int i_block = 1; i_block < N - 1; i_block += BLOCK_SIZE) {
        for (int j_block = 1; j_block < N - 1; j_block += BLOCK_SIZE) {
            for (int k_block = 1; k_block < N - 1; k_block += BLOCK_SIZE) {
                ...
                // Loops que iteram DENTRO de um bloco
                for (int i = i_block; i < i_block + BLOCK_SIZE && i < N - 1; ++i) {
                    for (int j = j_block; j < j_block + BLOCK_SIZE && j < N - 1; ++j) {
                        // A diretiva SIMD é aplicada ao loop mais interno (k)
                        // dentro do trabalho já distribuído. Esta estrutura é robusta.
                        #pragma omp simd
...
int main(int argc, char *argv[]) { /* ...código sem alterações... */ 
    if (argc != 2) {
